      ...
      if (cubes != null) {
        for (var c in cubes) {
         ...
         if (player_spacecraft != null){ 
            ...
            // COLLISION
            // with cubes
            if ( (bbox_cube.max.x >= bbox_spacecraft.max.x && bbox_cube.min.x   <= bbox_spacecraft.min.x) &&
                 (bbox_cube.max.y  >= bbox_spacecraft.max.y && bbox_cube.min.y   <= bbox_spacecraft.min.y) &&
                 (bbox_cube.max.z  >= bbox_spacecraft.max.z && bbox_cube.min.z   <= bbox_spacecraft.min.z) ) {
